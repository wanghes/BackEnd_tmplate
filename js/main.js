$(function(){
    //顶部点击后切换左边列表
    $('#main_menu .top_menu').click(function(){
        $('#main_menu li').removeClass('current');
        $(this).parent().addClass('current');
        var _sub_menu = $(this).parent('li').find('ul');
        if( _sub_menu.html() != undefined )
        {
            var _sub_menu_content = "<ul class=\"left_menu_wrap\">"+ _sub_menu.html() + '</ul>';
            $('#sub_menu').html(_sub_menu_content);
        }
        return false;
    });
    ...
    //打开新TAB页面
    $(document).on('click', '.left_menu_wrap a', function(e){
        ...
        $('.left_menu_wrap li').removeClass('current');
...
function switchPosition(_obj)
{
    ...
    {
        var _move_to = main_width - cur_pos - tab_width-20;
